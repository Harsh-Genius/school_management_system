...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
    ...
    if(!empty($email_id) && !empty($teacher_password) )
    {
        
        //read  from database
        
        $query="select * from teacher where email_id  = '$email_id' limit 1";
            ...
            if($result && mysqli_num_rows($result)>0)
        {
            ...
            if($user_data['teacher_password'] === $teacher_password)
            {   
                $_SESSION['teacher_class'] = $user_data['class'];
                $_SESSION['teacher_name'] = $user_data['teacher_name'];
                $_SESSION['email_id'] = $user_data['email_id'];
                $_SESSION['post'] = $user_data['post'];
                $_SESSION['phone_no'] = $user_data['phone_no'];
                $_SESSION['password'] = $user_data['teacher_password'];
...
<?php include 'menu.php'  ?>
    ...
    <div class="form">
        <form method="post">
           <h3><label>Enter Teacher Email Id</label></h3>
            <div style="">
            ...
            <br>
            <h3><label>Enter Teacher password</label></h3>
            <div style="">
                <input type="possword" class="" id="inputEmail3" placeholder="1234" name="teacher_password"
                    style="border-radius: 20px; width:100ex;  text-align: center; padding: 10px;">
            </div>
